...
description: >
  Conduct an Example Mapping session to explore a feature
  before TDD — discover rules, examples, and open questions
  collaboratively. Use BEFORE creating a test list.
when_to_use: >
  When the user wants to explore a feature before writing
  tests, discover business rules, or run an Example Mapping
  session. Examples: "Let's do an example mapping",
  "What are the rules for this feature?",
  "I want to explore the requirements first"
allowed-tools: Read Glob Grep AskUserQuestion Write
---

# Example Mapping

When this skill is triggered, conduct an interactive Example Mapping session with the user.
